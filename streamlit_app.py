...
if st.session_state.current_step == 1:
    ...
    needs_generation = (
        ...
        st.session_state.get('force_regenerate', False) or
        (last_signature != inputs_signature and st.session_state.get('allow_auto_regenerate', False))
    )
            ...
            with st.expander("🔧 Troubleshooting Guide", expanded=True):
                ...
                with action_col1:
                    if st.button("🔄 Retry Generation", key="retry_gen_btn"):
                        ...
                        st.rerun()
                with action_col2:
                    if st.button("⬅️ Go to Step 1", key="goto_step1_btn"):
    ...
    if st.session_state.pipeline_result:
        ...
        with col2:
            if st.button("🔄 Regenerate Content with Current Settings", key="manual_regenerate_btn", type="secondary", use_container_width=True):
                st.session_state.force_regenerate = True
                st.rerun()
            ...
            with tabs[0]:
                ...
                if not variants:
                    ...
                    with content_container:
                        ...
                        escaped_content = article_content.replace('\\', '\\\\').replace('`', '\\`').replace('$', '\\$')
                    ...
                    for idx, (platform, post) in enumerate(chosen_variant['social_posts'].items()):
                        with social_cols[idx % 2]:
                            with st.container():
                                ...
                                copy_post_button_html = f"""
                                <button onclick="copyPost{platform}()" style="
                                ...
                                " onmouseover="this.style.backgroundColor='#0b7dda'" 
                                   onmouseout="this.style.backgroundColor='#2196F3'">
                                    📋 Copy {platform.title()} Post
                                </button>
                                <span id="copy-status-{platform}" style="margin-left: 10px; color: green;"></span>
                                
                                <script>
                                function copyPost{platform}() {{
                                    const content = `{escaped_post_content}`;
                                    navigator.clipboard.writeText(content).then(function() {{
                                        document.getElementById('copy-status-{platform}').innerText = '✅ Copied!';
                                        setTimeout(function() {{
                                            document.getElementById('copy-status-{platform}').innerText = '';
                                        }}, 3000);
                                    }}, function(err) {{
                                        document.getElementById('copy-status-{platform}').innerText = '❌ Failed';
                                        console.error('Could not copy text: ', err);
                                    }});
                                }}
                                </script>
